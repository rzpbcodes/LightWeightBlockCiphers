class LED_64_32:
    ...
    @classmethod
    def uintArr2byteCopy(cls, x, b, offSet):
        b[0 + offSet] = (x[0] & 0xFF)
        b[2 + offSet] = (x[0] >> 8 & 0xFF)
        b[4 + offSet] = (x[0] >> 16 & 0xFF)
        b[6 + offSet] = (x[0] >> 24 & 0xFF)

        b[1 + offSet] = (x[1] & 0xFF)
        b[3 + offSet] = (x[1] >> 8 & 0xFF)
        b[5 + offSet] = (x[1] >> 16 & 0xFF)
        b[7 + offSet] = (x[1] >> 24 & 0xFF)
    ...
    def AddConstants(cls, state, round):
        state[0] ^= cls.RCandKeySizeConst[round]
        return

    @classmethod
    def SubCellShiftRowAndMixColumns(cls, state):
        temp = [None]*2
        temp[0] = cls.T0[state[0] & cls.mask]
        temp[0] ^= cls.T1[((state[0] >> 12) & 0x0000000F) ^ ((state[1] >> 4) & 0x000000F0)]
        temp[0] ^= cls.T2[(state[1] >> 16) & cls.mask]
        temp[0] ^= cls.T3[(((state[0] >> 20) & 0x000000F0) ^ ((state[1] >> 28) & 0x0000000F)) & cls.mask]

        temp[1] = cls.T0[state[1] & cls.mask]
        temp[1] ^= cls.T1[((state[1] >> 12) & 0x0000000F) ^ ((state[0] >> 4) & 0x000000F0)]
        temp[1] ^= cls.T2[(state[0] >> 16) & cls.mask]
        temp[1] ^= cls.T3[(((state[1] >> 20) & 0x000000F0) ^ ((state[0] >> 28) & 0x0000000F)) & cls.mask]
    ...
    def Encrypt(cls, plainText, key):
        ...
        sk0 = cls.byte2uintArr(key, 0)
        for i in range(0, len(plainText), 8):
            state = cls.byte2uintArr(plainText, i)
            cls.EncryptOneBlock(state, sk0)
            cls.uintArr2byteCopy(state, cipherText, i)
        return cipherText
