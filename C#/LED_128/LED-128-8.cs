using System;
using System.Collections.Generic;

using System.Text;

namespace LW_BlockCiphersCSharp
{
    class LED_128_8
    {
        static byte keySize = 80;

        static byte[] RC0 = new byte[] { 0x08, 0x08, 0x08, 0x18, 0x38, 0x78, 0x78, 0x78, 0x68, 0x58, 0x38, 0x78, 0x78, 0x68, 0x48, 0x18, 0x38, 0x78, 0x68, 0x58, 0x28, 0x58, 0x38, 0x68, 0x48, 0x08, 0x08, 0x18, 0x28, 0x58, 0x38, 0x78, 0x68, 0x48, 0x08, 0x18, 0x38, 0x68, 0x58, 0x38, 0x68, 0x58, 0x28, 0x48, 0x18, 0x28, 0x48, 0x08 };
        static byte[] RC2 = new byte[] { 0x19, 0x39, 0x79, 0x79, 0x79, 0x69, 0x59, 0x39, 0x79, 0x79, 0x69, 0x49, 0x19, 0x39, 0x79, 0x69, 0x59, 0x29, 0x59, 0x39, 0x69, 0x49, 0x09, 0x09, 0x19, 0x29, 0x59, 0x39, 0x79, 0x69, 0x49, 0x09, 0x19, 0x39, 0x69, 0x59, 0x39, 0x69, 0x59, 0x29, 0x49, 0x19, 0x29, 0x49, 0x09, 0x19, 0x29, 0x49 };
        static byte[] RC4 = new byte[] { 0x02, 0x02, 0x02, 0x12, 0x32, 0x72, 0x72, 0x72, 0x62, 0x52, 0x32, 0x72, 0x72, 0x62, 0x42, 0x12, 0x32, 0x72, 0x62, 0x52, 0x22, 0x52, 0x32, 0x62, 0x42, 0x02, 0x02, 0x12, 0x22, 0x52, 0x32, 0x72, 0x62, 0x42, 0x02, 0x12, 0x32, 0x62, 0x52, 0x32, 0x62, 0x52, 0x22, 0x42, 0x12, 0x22, 0x42, 0x02 };
        static byte[] RC6 = new byte[] { 0x13, 0x33, 0x73, 0x73, 0x73, 0x63, 0x53, 0x33, 0x73, 0x73, 0x63, 0x43, 0x13, 0x33, 0x73, 0x63, 0x53, 0x23, 0x53, 0x33, 0x63, 0x43, 0x03, 0x03, 0x13, 0x23, 0x53, 0x33, 0x73, 0x63, 0x43, 0x03, 0x13, 0x33, 0x63, 0x53, 0x33, 0x63, 0x53, 0x23, 0x43, 0x13, 0x23, 0x43, 0x03, 0x13, 0x23, 0x43 };

        static byte[] sBox = new byte[] {  };
        static byte[] mul2 = new byte[] {  };
        static byte[] mul4 = new byte[] {  };
        static byte[] mul5 = new byte[] {  };
        static byte[] mul6 = new byte[] {  };
        static byte[] mul8 = new byte[] {  };
        static byte[] mul9 = new byte[] {  };
        static byte[] mula = new byte[] {  };
        static byte[] mulb = new byte[] {  };
        static byte[] mule = new byte[] {  };
        static byte[] mulf = new byte[] {  };


        static byte[] byte2byte(byte[] b, int offSet)
        {
            byte[] c = new byte[8];
            for (int i = 0; i < 8; i++)
                c[i] = b[offSet + i];
            return c;
        }

        static void byte2byte(byte[] state, byte[] b, int offSet)
        {
            for (int i = 0; i < 8; i++)
                b[offSet + i] = state[i];
        }

        static void AddKey(ref byte[] state, byte[] roundKey)
        {
            for (int i = 0; i < 8; i++)
                state[i] ^= roundKey[i];
        }

        static void AddConstants(ref byte[] state, int round)
        {
            state[0] ^= RC0[round];
            state[2] ^= RC2[round];
            state[4] ^= RC4[round];
            state[6] ^= RC6[round];
        }

        static void SubCellShiftRowAndMixColumns(ref byte[] state)
        {
            byte[] temp = new byte[8];
            byte b2, b3, b6, b7;

            b2 = (byte)((state[2] >> 4) ^ (state[3] << 4));
            b3 = (byte)((state[2] << 4) ^ (state[3] >> 4));

            b6 = (byte)((state[6] << 4) ^ (state[7] >> 4));
            b7 = (byte)((state[6] >> 4) ^ (state[7] << 4));

            temp[0] = (byte)(mul4[state[0]] ^ sBox[b2] ^ mul2[state[5]] ^ mul2[b6]);
            temp[2] = (byte)(mul8[state[0]] ^ mul6[b2] ^ mul5[state[5]] ^ mul6[b6]);
            temp[4] = (byte)(mulb[state[0]] ^ mule[b2] ^ mula[state[5]] ^ mul9[b6]);
            temp[6] = (byte)(mul2[state[0]] ^ mul2[b2] ^ mulf[state[5]] ^ mulb[b6]);

            temp[1] = (byte)(mul4[state[1]] ^ sBox[b3] ^ mul2[state[4]] ^ mul2[b7]);
            temp[3] = (byte)(mul8[state[1]] ^ mul6[b3] ^ mul5[state[4]] ^ mul6[b7]);
            temp[5] = (byte)(mulb[state[1]] ^ mule[b3] ^ mula[state[4]] ^ mul9[b7]);
            temp[7] = (byte)(mul2[state[1]] ^ mul2[b3] ^ mulf[state[4]] ^ mulb[b7]);

            state = temp;
        }

        static void Step(ref byte[] state, int step)
        {
            for (int i = 0; i < 4; i++)
            {
                AddConstants(ref state, (step * 4 + i));
                SubCellShiftRowAndMixColumns(ref state);
            }
        }

        static void EncryptOneBlock(ref byte[] state, byte[] sk0, byte[] sk1)
        {
            for (int i = 0; i < 12; i += 2)
            {
                AddKey(ref state, sk0);
                Step(ref state, i);
                AddKey(ref state, sk1);
                Step(ref state, i + 1);
            }
            AddKey(ref state, sk0);
        }

        public static byte[] Encrypt(byte[] input, byte[] key)
        {
            byte[] cipherText = new byte[input.Length];

            byte[] sk0 = byte2byte(key, 0);
            byte[] sk1 = byte2byte(key, 8);

            for (int i = 0; i < input.Length; i += 8)
            {
                byte[] state = byte2byte(input, i);

                EncryptOneBlock(ref state, sk0,sk1);

                byte2byte(state, cipherText, i);
            }

            return cipherText;
        }

    }
}