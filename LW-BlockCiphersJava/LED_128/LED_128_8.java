package lw.blockciphersjava.LED_128;

public class LED_128_8 {
    static short keySize = 128;

    static short[] RC0 = new short[] { 0x08, 0x08, 0x08, 0x18, 0x38, 0x78, 0x78, 0x78, 0x68, 0x58, 0x38, 0x78, 0x78, 0x68, 0x48, 0x18, 0x38, 0x78, 0x68, 0x58, 0x28, 0x58, 0x38, 0x68, 0x48, 0x08, 0x08, 0x18, 0x28, 0x58, 0x38, 0x78, 0x68, 0x48, 0x08, 0x18, 0x38, 0x68, 0x58, 0x38, 0x68, 0x58, 0x28, 0x48, 0x18, 0x28, 0x48, 0x08 };
    static short[] RC2 = new short[] { 0x19, 0x39, 0x79, 0x79, 0x79, 0x69, 0x59, 0x39, 0x79, 0x79, 0x69, 0x49, 0x19, 0x39, 0x79, 0x69, 0x59, 0x29, 0x59, 0x39, 0x69, 0x49, 0x09, 0x09, 0x19, 0x29, 0x59, 0x39, 0x79, 0x69, 0x49, 0x09, 0x19, 0x39, 0x69, 0x59, 0x39, 0x69, 0x59, 0x29, 0x49, 0x19, 0x29, 0x49, 0x09, 0x19, 0x29, 0x49 };
    static short[] RC4 = new short[] { 0x02, 0x02, 0x02, 0x12, 0x32, 0x72, 0x72, 0x72, 0x62, 0x52, 0x32, 0x72, 0x72, 0x62, 0x42, 0x12, 0x32, 0x72, 0x62, 0x52, 0x22, 0x52, 0x32, 0x62, 0x42, 0x02, 0x02, 0x12, 0x22, 0x52, 0x32, 0x72, 0x62, 0x42, 0x02, 0x12, 0x32, 0x62, 0x52, 0x32, 0x62, 0x52, 0x22, 0x42, 0x12, 0x22, 0x42, 0x02 };
    static short[] RC6 = new short[] { 0x13, 0x33, 0x73, 0x73, 0x73, 0x63, 0x53, 0x33, 0x73, 0x73, 0x63, 0x43, 0x13, 0x33, 0x73, 0x63, 0x53, 0x23, 0x53, 0x33, 0x63, 0x43, 0x03, 0x03, 0x13, 0x23, 0x53, 0x33, 0x73, 0x63, 0x43, 0x03, 0x13, 0x33, 0x63, 0x53, 0x33, 0x63, 0x53, 0x23, 0x43, 0x13, 0x23, 0x43, 0x03, 0x13, 0x23, 0x43 };

    static short[] sBox = new short[] {  };
    static short[] mul2 = new short[] {  };
    static short[] mul4 = new short[] {  };
    static short[] mul5 = new short[] {  };
    static short[] mul6 = new short[] {  };
    static short[] mul8 = new short[] {  };
    static short[] mul9 = new short[] {  };
    static short[] mula = new short[] {  };
    static short[] mulb = new short[] {  };
    static short[] mule = new short[] {  };
    static short[] mulf = new short[] {  };

    static short[] byte2byte(short[] b, int offSet)
    {
        short[] c = new short[8];
        for (int i = 0; i < 8; i++)
            c[i] = b[offSet + i];
        return c;
    }

    static void byte2byte(short[] state, short[] b, int offSet)
    {
        for (int i = 0; i < 8; i++)
            b[offSet + i] = state[i];
    }

    static void AddKey(short[] state, short[] roundKey)
    {
        for (int i = 0; i < 8; i++)
            state[i] ^= roundKey[i];
    }

    static void AddConstants(short[] state, int round)
    {
        state[0] ^= RC0[round];
        state[2] ^= RC2[round];
        state[4] ^= RC4[round];
        state[6] ^= RC6[round];
    }

    static void SubCellShiftRowAndMixColumns(short[] state)
    {
        short[] temp = new short[8];
        short b2, b3, b6, b7;

        b2 = (short) (((state[2] >> 4) ^ (state[3] << 4)) &0xFF);
        b3 = (short) (((state[2] << 4) ^ (state[3] >> 4)) &0xFF);

        b6 = (short) (((state[6] << 4) ^ (state[7] >> 4)) &0xFF);
        b7 = (short) (((state[6] >> 4) ^ (state[7] << 4)) &0xFF);

        temp[0] = (short)(mul4[state[0]] ^ sBox[b2] ^ mul2[state[5]] ^ mul2[b6]);
        temp[2] = (short)(mul8[state[0]] ^ mul6[b2] ^ mul5[state[5]] ^ mul6[b6]);
        temp[4] = (short)(mulb[state[0]] ^ mule[b2] ^ mula[state[5]] ^ mul9[b6]);
        temp[6] = (short)(mul2[state[0]] ^ mul2[b2] ^ mulf[state[5]] ^ mulb[b6]);

        temp[1] = (short)(mul4[state[1]] ^ sBox[b3] ^ mul2[state[4]] ^ mul2[b7]);
        temp[3] = (short)(mul8[state[1]] ^ mul6[b3] ^ mul5[state[4]] ^ mul6[b7]);
        temp[5] = (short)(mulb[state[1]] ^ mule[b3] ^ mula[state[4]] ^ mul9[b7]);
        temp[7] = (short)(mul2[state[1]] ^ mul2[b3] ^ mulf[state[4]] ^ mulb[b7]);

        for(int i=0;i<8;i++)
            state[i] = temp[i];
    }

    static void Step(short[] state, int step)
    {
        for (int i = 0; i < 4; i++)
        {
            AddConstants(state, (step * 4 + i));
            SubCellShiftRowAndMixColumns(state);
        }
    }

    static void EncryptOneBlock(short[] state, short[] sk0, short[] sk1)
    {
        for (int i = 0; i < 12; i+=2)
        {
            AddKey(state, sk0);
            Step(state, i);
            AddKey(state, sk1);
            Step(state, i+1);
        }
        AddKey(state, sk0);
    }

    public static short[] Encrypt(short[] input, short[] key)
    {
        short[] cipherText = new short[input.length];

        short[] sk0 = byte2byte(key, 0);
        short[] sk1 = byte2byte(key, 8);

        for (int i = 0; i < input.length; i += 8)
        {
            short[] state = byte2byte(input, i);

            EncryptOneBlock(state, sk0, sk1);

            byte2byte(state, cipherText, i);
        }

        return cipherText;
    }
}
