...
public class LED_64_4_D2
{
    ...
    static void invMixColumnShiftRow(byte[] state)
    {
        byte[] temp = new byte[16];
    ...
    static void invSbox(byte[] state)
    {
    ...
    {
        byte[] skMix = new byte[sk0.length];
        for(int i=0;i<sk0.length;i++)
            skMix[i] = sk0[i];
