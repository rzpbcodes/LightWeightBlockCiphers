...
unsigned char LED_64_4s::keySize = 64;
unsigned char LED_64_4s::sBox[] = {0x0C, 0x05, 0x06, 0x0B, 0x09, 0x00, 0x0A, 0x0D, 0x03, 0x0E, 0x0F, 0x08, 0x04, 0x07, 0x01, 0x02};
unsigned char LED_64_4s::RC[] = {0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3E, 0x3D, 0x3B, 0x37, 0x2F, 0x1E, 0x3C, 0x39, 0x33, 0x27, 0x0E, 0x1D, 0x3A, 0x35, 0x2B, 0x16, 0x2C, 0x18, 0x30, 0x21, 0x02, 0x05, 0x0B, 0x17, 0x2E, 0x1C, 0x38};
unsigned char LED_64_4s::nibbleFilter = 0x0F;
unsigned char LED_64_4s::ReductionPoly = 0x3;

	unsigned char* LED_64_4s::byte2nibble(unsigned char bytes[], int offSet)
	{
	...
	unsigned char* LED_64_4s::nibbles2byte(unsigned char nibbles[])
	{
	...
	void LED_64_4s::nibbles2byte(unsigned char nibbles[], unsigned char bytes[], int offSet)
	{
		for (int i = 0; i < 8; i++)
		{
    ...
    unsigned char LED_64_4s::xTimes(unsigned char a)
    {
        a <<= 1;
        if ((a & 0x10) == 0x10)
            a ^= ReductionPoly;
    ...
    unsigned char LED_64_4s::x2Times(unsigned char a)
    {
        a <<= 1;
        if ((a & 0x10) == 0x10)
            a ^= ReductionPoly;
	...
	void LED_64_4s::AddKey(unsigned char state[], unsigned char roundKey[])
	{
		for (int i = 0; i < 16; i++)
		{
			state[i] ^= roundKey[i];
	...
	void LED_64_4s::AddConstants(unsigned char state[], int round)
	{
	...
	void LED_64_4s::SboxShiftRows(unsigned char state[])
	{
	...
	void LED_64_4s::MultiplyColumn(unsigned char temp[])
    {
	...
	void LED_64_4s::MixColumns(unsigned char state[])
	{
		for (int r = 0; r < 4; r++)
        {
	...
	void LED_64_4s::Step(unsigned char state[], int step)
	{
		for (int i = 0; i < 4; i++)
		{
	...
	void LED_64_4s::EncryptOneBlock(unsigned char state[], unsigned char sk0[])
	{
		for (int i = 0; i < 8; i++)
		{
			AddKey(state, sk0);
	...
	unsigned char* LED_64_4s::Encrypt(unsigned char input[], unsigned char key[], int len)
	{
