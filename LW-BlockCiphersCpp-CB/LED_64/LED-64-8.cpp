...
unsigned char LED_64_8::mulf[] = {};
	...
	unsigned char* LED_64_8::byte2byte(unsigned char b[], int offSet)
	{
	...
	void LED_64_8::byte2byte(unsigned char state[], unsigned char b[], int offSet)
	{
		for (int i = 0; i < 8; i++)
		{
			b[offSet + i] = state[i];
	...
	void LED_64_8::AddKey(unsigned char state[], unsigned char roundKey[])
	{
		for (int i = 0; i < 8; i++)
		{
			state[i] ^= roundKey[i];
	...
	void LED_64_8::AddConstants(unsigned char state[], int round)
	{
		state[0] ^= RC0[round];
		state[2] ^= RC2[round];
		state[4] ^= RC4[round];
	...
	void LED_64_8::SubCellShiftRowAndMixColumns(unsigned char state[])
	{
	...
	void LED_64_8::Step(unsigned char state[], int step)
	{
		for (int i = 0; i < 4; i++)
		{
	...
	void LED_64_8::EncryptOneBlock(unsigned char state[], unsigned char sk0[])
	{
		for (int i = 0; i < 8; i++)
		{
			AddKey(state, sk0);
	...
	unsigned char* LED_64_8::Encrypt(unsigned char input[], unsigned char key[], int len)
	{
