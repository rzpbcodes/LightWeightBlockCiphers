#include "LED-80-8.h"

namespace LW_BlockCiphers
{

unsigned char LED_80_8::keySize = 80;

unsigned char LED_80_8::RC0[] = { 0x05, 0x05, 0x05, 0x15, 0x35, 0x75, 0x75, 0x75, 0x65, 0x55, 0x35, 0x75, 0x75, 0x65, 0x45, 0x15, 0x35, 0x75, 0x65, 0x55, 0x25, 0x55, 0x35, 0x65, 0x45, 0x05, 0x05, 0x15, 0x25, 0x55, 0x35, 0x75, 0x65, 0x45, 0x05, 0x15, 0x35, 0x65, 0x55, 0x35, 0x65, 0x55, 0x25, 0x45, 0x15, 0x25, 0x45, 0x05 };
unsigned char LED_80_8::RC2[] = { 0x14, 0x34, 0x74, 0x74, 0x74, 0x64, 0x54, 0x34, 0x74, 0x74, 0x64, 0x44, 0x14, 0x34, 0x74, 0x64, 0x54, 0x24, 0x54, 0x34, 0x64, 0x44, 0x04, 0x04, 0x14, 0x24, 0x54, 0x34, 0x74, 0x64, 0x44, 0x04, 0x14, 0x34, 0x64, 0x54, 0x34, 0x64, 0x54, 0x24, 0x44, 0x14, 0x24, 0x44, 0x04, 0x14, 0x24, 0x44 };
unsigned char LED_80_8::RC4[] = { 0x02, 0x02, 0x02, 0x12, 0x32, 0x72, 0x72, 0x72, 0x62, 0x52, 0x32, 0x72, 0x72, 0x62, 0x42, 0x12, 0x32, 0x72, 0x62, 0x52, 0x22, 0x52, 0x32, 0x62, 0x42, 0x02, 0x02, 0x12, 0x22, 0x52, 0x32, 0x72, 0x62, 0x42, 0x02, 0x12, 0x32, 0x62, 0x52, 0x32, 0x62, 0x52, 0x22, 0x42, 0x12, 0x22, 0x42, 0x02 };
unsigned char LED_80_8::RC6[] = { 0x13, 0x33, 0x73, 0x73, 0x73, 0x63, 0x53, 0x33, 0x73, 0x73, 0x63, 0x43, 0x13, 0x33, 0x73, 0x63, 0x53, 0x23, 0x53, 0x33, 0x63, 0x43, 0x03, 0x03, 0x13, 0x23, 0x53, 0x33, 0x73, 0x63, 0x43, 0x03, 0x13, 0x33, 0x63, 0x53, 0x33, 0x63, 0x53, 0x23, 0x43, 0x13, 0x23, 0x43, 0x03, 0x13, 0x23, 0x43 };

unsigned char LED_80_8::sBox[] = {};
unsigned char LED_80_8::mul2[] = {};
unsigned char LED_80_8::mul4[] = {};
unsigned char LED_80_8::mul5[] = {};
unsigned char LED_80_8::mul6[] = {};
unsigned char LED_80_8::mul8[] = {};
unsigned char LED_80_8::mul9[] = {};
unsigned char LED_80_8::mula[] = {};
unsigned char LED_80_8::mulb[] = {};
unsigned char LED_80_8::mule[] = {};
unsigned char LED_80_8::mulf[] = {};

	unsigned char* LED_80_8::byte2byte(unsigned char b[], int offSet)
	{
		unsigned char* c = new unsigned char[8];
		for (int i = 0; i < 8; i++)
		{
			c[i] = b[offSet + i];
		}
		return c;
	}

	void LED_80_8::byte2byte(unsigned char state[], unsigned char b[], int offSet)
	{
		for (int i = 0; i < 8; i++)
		{
			b[offSet + i] = state[i];
		}
	}

	void LED_80_8::AddKey(unsigned char state[], unsigned char roundKey[])
	{
		for (int i = 0; i < 8; i++)
		{
			state[i] ^= roundKey[i];
		}
	}

	void LED_80_8::AddConstants(unsigned char state[], int round)
	{
		state[0] ^= RC0[round];
		state[2] ^= RC2[round];
		state[4] ^= RC4[round];
		state[6] ^= RC6[round];
	}

	void LED_80_8::SubCellShiftRowAndMixColumns(unsigned char state[])
	{
		unsigned char* temp = new unsigned char[8];
		unsigned char b2, b3, b6, b7;

		b2 = static_cast<unsigned char>((state[2] >> 4) ^ (state[3] << 4));
		b3 = static_cast<unsigned char>((state[2] << 4) ^ (state[3] >> 4));

		b6 = static_cast<unsigned char>((state[6] << 4) ^ (state[7] >> 4));
		b7 = static_cast<unsigned char>((state[6] >> 4) ^ (state[7] << 4));

		temp[0] = static_cast<unsigned char>(mul4[state[0]] ^ sBox[b2] ^ mul2[state[5]] ^ mul2[b6]);
		temp[2] = static_cast<unsigned char>(mul8[state[0]] ^ mul6[b2] ^ mul5[state[5]] ^ mul6[b6]);
		temp[4] = static_cast<unsigned char>(mulb[state[0]] ^ mule[b2] ^ mula[state[5]] ^ mul9[b6]);
		temp[6] = static_cast<unsigned char>(mul2[state[0]] ^ mul2[b2] ^ mulf[state[5]] ^ mulb[b6]);

		temp[1] = static_cast<unsigned char>(mul4[state[1]] ^ sBox[b3] ^ mul2[state[4]] ^ mul2[b7]);
		temp[3] = static_cast<unsigned char>(mul8[state[1]] ^ mul6[b3] ^ mul5[state[4]] ^ mul6[b7]);
		temp[5] = static_cast<unsigned char>(mulb[state[1]] ^ mule[b3] ^ mula[state[4]] ^ mul9[b7]);
		temp[7] = static_cast<unsigned char>(mul2[state[1]] ^ mul2[b3] ^ mulf[state[4]] ^ mulb[b7]);

		for(int i=0;i<8;i++)
            state[i] = temp[i];
	}

	void LED_80_8::Step(unsigned char state[], int step)
	{
		for (int i = 0; i < 4; i++)
		{
			AddConstants(state, (step * 4 + i));
			SubCellShiftRowAndMixColumns(state);
		}
	}

	void LED_80_8::EncryptOneBlock(unsigned char state[], unsigned char sk0[], unsigned char sk1[])
	{
		for (int i = 0; i < 12; i+=2)
		{
			AddKey(state, sk0);
			Step(state, i);
			AddKey(state, sk1);
			Step(state, i+1);
		}
		AddKey(state, sk0);
	}

	unsigned char* LED_80_8::Encrypt(unsigned char input[], unsigned char key[], int len)
	{
		unsigned char* cipherText = new unsigned char[len];

		unsigned char* sk0 = byte2byte(key, 0);
		unsigned char *sk2 = new unsigned char[8];
		for(int i=0;i<8;i++)
            sk2[i] = key[(8+i)%10];
		unsigned char* sk1 = byte2byte(sk2, 0);

		for (int i = 0; i < len; i += 8)
		{
			unsigned char* state = byte2byte(input, i);

			EncryptOneBlock(state, sk0, sk1);

			byte2byte(state, cipherText, i);
		}

		return cipherText;
	}
}
