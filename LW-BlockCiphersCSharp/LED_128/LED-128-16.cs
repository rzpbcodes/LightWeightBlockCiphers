...
namespace LW_BlockCiphersCSharp
{
    class LED_128_16
    {
        ...
        static UInt16 keySizeConst = 0x3298;
        ...
        static UInt16[] byte2uint16(byte[] bytes, int offSet)
        {
            UInt16[] x = new UInt16[4];
            x[0] ^= (UInt16)((bytes[offSet + 0] & 0x0F) << 0);
            x[0] ^= (UInt16)((bytes[offSet + 2] & 0x0F) << 4);
            x[0] ^= (UInt16)((bytes[offSet + 4] & 0x0F) << 8);
            x[0] ^= (UInt16)((bytes[offSet + 6] & 0x0F) << 12);

            x[1] ^= (UInt16)((bytes[offSet + 0] & 0xF0) >> 4);
            x[1] ^= (UInt16)((bytes[offSet + 2] & 0xF0) << 0);
            x[1] ^= (UInt16)((bytes[offSet + 4] & 0xF0) << 4);
            x[1] ^= (UInt16)((bytes[offSet + 6] & 0xF0) << 8);

            x[2] ^= (UInt16)((bytes[offSet + 1] & 0x0F) << 0);
            x[2] ^= (UInt16)((bytes[offSet + 3] & 0x0F) << 4);
            x[2] ^= (UInt16)((bytes[offSet + 5] & 0x0F) << 8);
            x[2] ^= (UInt16)((bytes[offSet + 7] & 0x0F) << 12);

            x[3] ^= (UInt16)((bytes[offSet + 1] & 0xF0) >> 4);
            x[3] ^= (UInt16)((bytes[offSet + 3] & 0xF0) << 0);
            x[3] ^= (UInt16)((bytes[offSet + 5] & 0xF0) << 4);
            x[3] ^= (UInt16)((bytes[offSet + 7] & 0xF0) << 8);
        ...
        static void EncryptOneBlock(ref UInt16[] state, UInt16[] sk0, UInt16[] sk1)
        {
